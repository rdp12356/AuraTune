...
const FADE_DURATION = AUDIO_CONFIG.FADE_DURATION;
const FREQUENCY_LOUDNESS_BOOST = AUDIO_CONFIG.FREQUENCY_LOUDNESS_BOOST;
...
function ignoreCatch() {
  // Explicit no-op for non-fatal audio API errors.
}
...
// Cleanup function to clear noise buffer cache (call on app cleanup)
export function cleanupAudioEngine() {
  stopAll();
  stopKeepalive();
  noiseBufferCache.clear();
  if (audioCtx?.state !== 'closed') {
    void audioCtx?.close().catch(ignoreCatch);
    audioCtx = null;
  }
}
